...
const ActionButtons: React.FC<ActionButtonsProps> = ({
  ...
  // 获取按钮文本
  const getButtonText = () => {
    if (isLoading) return "融合中...";
    if (hasResult) return "重新融合";
    if (canPerformAction) return "开始融合";
    return "请选择表情";
  };
      ...
      <div className="flex justify-center relative">
        ...
        <div
          className={`absolute w-20 h-20 rounded-full bg-gradient-to-br from-blue-500/40 to-white-500/40 blur-lg
                      ${isRandomHovered ? 'opacity-80' : 'opacity-0'} transition-opacity duration-300`}
        ...
        <div className="relative">
          <button
            ...
            className={`
              ...
              ${canPerformAction && !isLoading
                ? hasResult
                  ? "bg-gradient-to-r from-secondary-500/80 to-secondary-600/80 hover:from-secondary-400/80 hover:to-secondary-500/80 text-white hover:scale-105"
                  : "bg-gradient-to-r from-primary-500/80 to-primary-600/80 hover:from-primary-400/80 hover:to-primary-500/80 text-white hover:scale-105"
